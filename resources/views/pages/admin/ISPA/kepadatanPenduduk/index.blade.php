                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    @forelse ($kepadatanPendudukIspa as $index => $item)
                                        <tr>
                                            <td>{{ ($kepadatanPendudukIspa->currentPage() - 1) * $kepadatanPendudukIspa->perPage() + $index + 1 }}
                                            </td>
                                            <td>
                                                @if ($item->gambar)
                                                    <img src="{{ asset('storage/kepadatanPendudukIspa/' . $item->gambar) }}"
                                                        alt="gambar"
                                            ...
                                            <td>{{ $item->kpdt_bps }}</td>
                                            <td>{{ $item->kelas_kpdt }}</td>
                                            <td>{{ $item->tanggal }}</td>
                                            <td>{{ $item->operator }}</td>
                                            <td>
                                                <a href="{{ route('admin.kepadatan-penduduk-ispa.edit', $item->id) }}"
                                                    class="btn btn-warning btn-sm">Edit</a>
                                                <form
                                                    action="{{ route('admin.kepadatan-penduduk-ispa.destroy', $item->id) }}"
                        ...
                        <div class="mt-3">
                            {{ $kepadatanPendudukIspa->links('vendor.pagination.bootstrap-5') }}
                        </div>
                    </div>
                </div>
            </div>
